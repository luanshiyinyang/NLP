...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self._state_size, self._state_size], name="W_r")
            U_r = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self.input_size, self._state_size], name="U_r")
            b_r = tf.get_variable('b_r', [self._state_size, ], initializer=tf.constant_initializer(0), dtype=tf.float32)
            W_z = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self._state_size, self._state_size], name="W_z")
            U_z = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self.input_size, self._state_size], name="U_z")
            b_z = tf.get_variable('b_z', [self._state_size, ], initializer=tf.constant_initializer(0), dtype=tf.float32)
            W_o = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self._state_size, self._state_size], name="W_o")
            U_o = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[self.input_size, self._state_size], name="U_o")
            b_o = tf.get_variable('b_o', [self._state_size, ], initializer=tf.constant_initializer(0), dtype=tf.float32)
